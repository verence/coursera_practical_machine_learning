---
title: "Machine Lerning - Course Project "
author: "Verence"
date: "9 März 2017"
output: 
  html_document:
...
Both data frames have 159 posible predictors and one output.
...
# Model Bilding #
The training data has 159 posible predictors and one output variable which is the *class* variable.
## finding significants *
I start to find significant variable for prediction.
Analysing the *training* data I notice that a few cloumns contains NA. Columns with many NA are not veriy significant for predicting.
...
Following  is a list of top twenty predictors odered by percentUnique desc.
...
By selecting variable as predictor I used following creterias:
...
+ Variabels schould be independent by each other.
...
Looking at the table above you can see how the rmse goes down be include more and more predictor variables in the model.
To avoid overfitting only pitch_arm + yaw_arm + pitch_dumbbell + yaw_dumbbell + pitch_forearm + yaw_forearm will selected as predictos in the final model.
...
The accuracy on *test* data will be `r 0.028`. It is some points worst than the accuracy on *training* data.

# Results #
The created model inclule 8 variables as predictors and reach an accuracy of `r 0.028` on *training* data and an accuracy on *training* data of.
